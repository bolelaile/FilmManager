...
- **合并 v1.1.81 开发成果** — 整合卷视图筛选、建卷属性一致性检查、单批次导入建卷、设置页存储路径/关于/日志，以及胶片和相机 JSON 预设。
- **拍摄地点批量编辑** — 单张或多选照片均可从右键菜单设置、更换或清除拍摄地点，与属性编辑共用统一入口。
- **地点地图增强** — 扩充国内城市、区县和摄影目的地预设；地图瓦片按 OSM.de、Esri、标准 OpenStreetMap 自动故障切换，并提供失败状态与重试。
- **缩略图布局修复** — 修复照片加载后容器宽度未重新测量导致的大缩略图挤在左侧问题，小、中、大三档视图均能随窗口尺寸正确排版。
- **依赖与类型稳定性** — 升级 `exif-reader` 至 2.0.3，移除未使用且与 React 18 冲突的 `react-leaflet`，修复 preload 工程边界和 Electron 标题栏 CSS 类型声明。
...
| 1.2.0 | 合并 v1.1.81；新增拍摄地点批量设置/清除、地图多瓦片源容错与地点预设，修复大缩略图响应式布局，并解决 React/Leaflet 依赖冲突和 Web TypeScript 基线错误 |
