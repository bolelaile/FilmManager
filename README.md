...
| 1.1.8 | 卷视图三项优化：① 构建卷时验证照片胶卷类型与相机型号一致性，不一致时显示警告（CreateRollModal 阻断确认、单批次导入建卷显示提示）；② 左侧导航栏自动更新——建卷后同步刷新属性计数；③ 卷视图下左侧属性筛选器生效，按胶卷/相机等属性过滤卷显示 |
